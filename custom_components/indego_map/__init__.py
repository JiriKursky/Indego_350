...
from .const import (
    ...
    ENTITY_MOWER_POSITION,
    INDEGO_COMPONENTS,
    SENSOR_TYPE,
    SERVICE_NAME_COMMAND,
    SERVICE_NAME_SMARTMOW,
    SERVICE_NAME_UPDATE_STATE,
...
ENTITY_DEFINITIONS = {
    ...
    ENTITY_MOWER_POSITION: {
        ...
        CONF_DEVICE_CLASS: DEVICE_CLASS_TIMESTAMP,
        CONF_UNIT_OF_MEASUREMENT: "ISO8601",
        CONF_ATTR: ["x_pos", "y_pos", "svg_x_pos", "svg_y_pos", "map_update_available"],
    },
}
...
async def async_setup(hass, config: dict):
    ...
    async def async_update_state(call):
        """Handle the service call. Be careful for using that - taking time"""
        _LOGGER.debug("Indego.update_state service called")
        await hass.data[DOMAIN]._update_state(True)
...
class IndegoHub:
    ...
    async def _initial_update(self, _):
        ...
        self.refresh_state_task = self._hass.async_create_task(self.refresh_state(True))
    ...
    async def refresh_state(self, force: bool = False):
        ...
        try:
            await self._update_state(force)
        except Exception as e:
    ...
    async def _update_state(self, force: bool = False):
        """Asking for update via API

        Args:
            force (bool, optional): True means sending refresh - necessary to get fresh positions. Defaults to False.
        """
        await self.indego.update_state(force=force, longpoll=True, longpoll_timeout=300)
        # dependent state updates
        if self._shutdown:
            return
        if self.indego.state:
            ...
            if force:
                # Update of positions
                self.entities[ENTITY_MOWER_POSITION].state = datetime.datetime.now()
                self.entities[ENTITY_MOWER_POSITION].add_attribute(
                    {
                        "x_pos": self.indego.state.xPos,
                        "y_pos": self.indego.state.yPos,
                        "svg_x_pos": self.indego.state.svg_xPos,
                        "svg_y_pos": self.indego.state.svg_yPos,
                        "map_update_available": self.indego.state.map_update_available,
                    }
                )
